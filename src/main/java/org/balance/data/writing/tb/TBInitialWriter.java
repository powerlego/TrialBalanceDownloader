 ...
 */
public class TBInitialWriter extends Writer {

    /**
     * The instance of the logger
     */
    private static final Logger logger = LogManager.getLogger(TBInitialWriter.class);

    private final Balances balances;
    private final Task<?> task;

    public TBInitialWriter(Path workbookFile, Balances balances, Task<?> task) {
        this(workbookFile.toFile(), balances, task);
    }

    public TBInitialWriter(File workbookFile, Balances balances, Task<?> task) {
        super(workbookFile);
    ...
    public void makeWorkbook() {
        ...
        for (String accountNum : accountNums) {
            ...
            row.createCell(0).setCellValue(accountNum);
